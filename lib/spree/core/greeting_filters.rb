module Spree
  module Core
    ...
    #      require 'spree/core/greeting_filters'
    #

    # set up some basic filters for use with greetings
    #
    ...
    #
    # The details of how/when filters are used is a detail for specific models (eg greetings
    ...
    module GreetingFilters
      ...
      #   'price' is in the base scope's context (ie, "select foo from greetings where ...") so
      ...
      Spree::Greeting.add_search_scope :brand_any do |*opts|
        conds = opts.map {|o| GreetingFilters.brand_filter[:conds][o]}.reject { |c| c.nil? }
        scope = conds.shift
        conds.each do |new_scope|
          scope = scope.or(new_scope)
        end
        Spree::Greeting.with_property('brand').where(scope)
      end

      def GreetingFilters.brand_filter
        brand_property = Spree::Property.find_by(name: 'brand')
        brands = brand_property ? Spree::GreetingProperty.where(property_id: brand_property.id).pluck(:value).uniq.map(&:to_s) : []
        pp = Spree::GreetingProperty.arel_table
      ...
      #   the greeting properties model.
      Spree::Greeting.add_search_scope :selective_brand_any do |*opts|
        Spree::Greeting.brand_any(*opts)
      end

      def GreetingFilters.selective_brand_filter(taxon = nil)
        taxon ||= Spree::Taxonomy.first.root
        brand_property = Spree::Property.find_by(name: 'brand')
        scope = Spree::GreetingProperty.where(property: brand_property).
          joins(greeting: :taxons).
      ...
      #
      # This scope selects greetings in any of the active taxons or their children.
      #
      ...
      #
      # Similar idea as above, but we don't want the descendants' greetings, hence
